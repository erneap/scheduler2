...
@Component({
  selector: 'app-site-file-ingest-month-employee-day',
  templateUrl: './site-file-ingest-month-employee-day.component.html',
  styleUrls: ['./site-file-ingest-month-employee-day.component.scss']
})
export class SiteFileIngestMonthEmployeeDayComponent {
  ...
  onChange() {
    ...
    if (numRe.test(value) && value !== this._currentValue) {
      ...
      if (found) {
        ...
        this.authService.statusMessage = 'Manual Entry Problem: '
          + `${this.employee.name.last}: ${this.date.getDate()} `
          + `- Illegal Entry Value`;
      }
    }
  }
